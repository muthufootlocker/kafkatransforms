...
public abstract class InnerDateCustom<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    public static final String OVERVIEW_DOC =
            "Convert timestamps between different formats such as Unix epoch, strings, and Connect Date/Timestamp types."
                    ...
                    + "<p/>Use the concrete transformation type designed for the record key (<code>" + InnerDateCustom.Key.class.getName() + "</code>) "
                    + "or value (<code>" + InnerDateCustom.Value.class.getName() + "</code>).";
    ...
    private static final Map<String, InnerDateCustom.TimestampTranslator> TRANSLATORS = new HashMap<>();

    static {
        TRANSLATORS.put(TYPE_STRING, new InnerDateCustom.TimestampTranslator() {
            @Override
            public Date toRaw(InnerDateCustom.Config config, Object orig) {
            ...
            @Override
            public String toType(InnerDateCustom.Config config, Date orig) {
        ...
        TRANSLATORS.put(TYPE_UNIX, new InnerDateCustom.TimestampTranslator() {
            @Override
            public Date toRaw(InnerDateCustom.Config config, Object orig) {
            ...
            @Override
            public Long toType(InnerDateCustom.Config config, Date orig) {
        ...
        TRANSLATORS.put(TYPE_DATE, new InnerDateCustom.TimestampTranslator() {
            @Override
            public Date toRaw(InnerDateCustom.Config config, Object orig) {
            ...
            @Override
            public Date toType(InnerDateCustom.Config config, Date orig) {
        ...
        TRANSLATORS.put(TYPE_TIME, new InnerDateCustom.TimestampTranslator() {
            @Override
            public Date toRaw(InnerDateCustom.Config config, Object orig) {
                if (!(orig instanceof Date))
        ...
        TRANSLATORS.put(TYPE_TIMESTAMP, new InnerDateCustom.TimestampTranslator() {
            @Override
            public Date toRaw(InnerDateCustom.Config config, Object orig) {
                if (!(orig instanceof Date))
    ...
    private InnerDateCustom.Config config;
    ...
    public void configure(Map<String, ?> configs) {
        ...
        config = new InnerDateCustom.Config(field, type, format, unixPrecision);
    }
    ...
    private void convertArrayOfObject(Map<String, Object> json, String jsonPath) {
        try {
            ...
            //throw new ConnectException("Path not found while converting the array of object in InnerDateCustom.class");
        }
    }
...
// check if the current value is a string that can be parsed as a date
    ...
    private Object convertTimestamp(Object timestamp, String timestampFormat) {
        ...
        InnerDateCustom.TimestampTranslator sourceTranslator = TRANSLATORS.get(timestampFormat);
        ...
        InnerDateCustom.TimestampTranslator targetTranslator = TRANSLATORS.get(config.type);
    ...
    private interface TimestampTranslator {
         ...
         */
        Date toRaw(InnerDateCustom.Config config, Object orig);
         ...
         */
        Object toType(InnerDateCustom.Config config, Date orig);
    }
    ...
    public static class Key<R extends ConnectRecord<R>> extends InnerDateCustom<R> {
        @Override
    ...
    public static class Value<R extends ConnectRecord<R>> extends InnerDateCustom<R> {
        @Override
        protected Schema operatingSchema(R record) {
